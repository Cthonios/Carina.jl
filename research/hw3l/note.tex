...
composite-tetrahedron and variational-multiscale families of methods carry. Three
...
theorem. Brezzi's theorem for the mixed problem asks two things of
...
those modes do not survive assembly.
...
Read as a constant, the second hypothesis cannot separate pairs. For every $~v \in H^1_0(B)$,
...
Equation \eqref{eq:hw} is a form of an established construction, and
identifying it is a precondition for stating what in the formulation is new.
...
quadratic $W\supvol$: the exactness is a property of the discretization, that
the two auxiliary fields share one space, and not of the material. What the
material must supply is set out in \S\ref{sec:matreq}: an exact
volumetric--isochoric split, without which the subtraction leaves unprojected
volumetric content inside the deviatoric term, which then locks. The
element-level implementation in Norma refuses materials that fail it.
...
And it means this formulation shares its mechanism with the $\bar{F}$ family
of elements it is meant to improve on. The claim is not that the mechanism is new; it is
that
...
strength. They are separated below, together with a third condition that is
not a requirement of this section.
...
necessity. Stationarity in $\bar p$ gives $\bar\theta = \Pi_h\theta$ regardless of
...
problem; they are different elements, and they differ by tens of percent
...
because $\log\C$ is the gradient of $\trace(\C\log\C - \C)$. Writing
$\mathbb{L}\supdev : \sdev$ in its place adds $-\tfrac13(\C^{-1}:\sdev)\F$,
which is not zero, since $\trace\sdev = 0$ does not make $\C^{-1}:\sdev$
vanish; the error is invisible in elasticity, where \eqref{eq:P-general}
collapses to \eqref{eq:P-simple}.
...
tensor for exactly this reason.
\end{remark}
...
\S\ref{sec:assembled} shows that its modes do not survive assembly. What
...
Taylor--Hood on $0.2216$, and every other element-local pair decays. The
difference between them is not stability but where the coupling is --- a global pressure
...
iteration \citep[BOX~3.2]{SimoHughes:1998}. Internal variables live at quadrature points and
...
Against this, a $P_2$ mesh resolves a given
...
with plasticity $\bar\sdev = \Pi_h\,\sdev(\bar\edev)$ is projected. Placing $\bar\edev$ and $\bar\sdev$ in different spaces ---
...
\eqref{eq:condensed} for the census and the locking sweep, the $H^1$ seminorm
...
zero-energy on one element, not merely soft. Whether they survive assembly is
...
affine counts there as well; the census uses a $27$-point rule. The assembled
...
estimate survives refinement; \S\ref{sec:beta} measures it.
\end{remark}
...
The restriction to the mesh family is necessary. On a general tetrahedral mesh with
...
The count has precedents. $n_p/n_u$ is the constraint
...
Whether that limiting degeneracy is the decisive objection in practice is a
separate question, which \S\ref{sec:assembled} answers.
...
The exact-zero soft modes of a constant pressure therefore do not survive
assembly on these elements. What this test cannot see is the mode
\citet{Foulk.etal:2021} report, which is remedied there by a penalty and therefore has energy --- too
...
over-constrains, and locking is a more damaging failure in practice than a
spurious mode, because it produces no symptom: the element returns a stiff
response.

\paragraph{The interior bubble raises the rank of $~G$ over a linear pressure.}
The interior bubble vanishes on the element boundary, so its columns of $~G$
against a constant pressure are zero (Remark~\ref{rem:whyface}) and it adds
nothing to the rank over $P_0$. Against the linear pressure modes its columns
...
full enrichment reaches $0.61$--$0.63$, in line with $P_0$ and without
$P_0$'s element-level degeneracy; it attains the bound $1 - n_p/n_u$ to three
digits and the interior-only pair to two, so no pressure unknown is spent
twice; and for the full pair $n_p - \operatorname{rank}~G = 1$ at every mesh,
the hydrostatic mode alone, where $P_2/P_0$ and the interior-only pair carry
four. That, rather than the soft-mode argument, is what the enrichment is
for.
...
sufficient is a different question, and the answer is no.
...
predict. What the sweep establishes is that the test bench is sound and that the theory
...
off the reference configuration than on it. Remark~\ref{rem:structured}
allows that a structured mesh might show the pair at its worst; its non-affine
images show it worse.
...
\subsection{Eigenvalue sweep with a negative control}
...
explains by counting. The dense eigensolver caps the sequence at $N = 5$,
over which the $P_2/P_0$ decay could be pre-asymptotic; \S\ref{sec:beta}
extends it to $N = 8$ with the singular-value formulation and it is not. What
this table shows that Table~\ref{tab:beta} also carries is the zero count of
\emph{four} for $P_2/P_0$ at every $N$: one is the hydrostatic mode,
...
\begin{enumerate}
  ...
  \item \textbf{No soft modes.} \emph{Closed in elasticity; in plasticity,
        ...
        reported. The consistent tangent at a fixed plastic state is a linear
        operator, and \S\ref{sec:plastic} measures on it the constant, which \eqref{eq:floor}
...
perform poorly: punch indentation, a nearly-incompressible Cook's membrane in three
...
\emph{The number of unknowns of the pair.} $P_2 \oplus b_{\mathrm{int}}
\oplus b_{\mathrm{face}}$ over $P_1^{\mathrm{disc}}$ is stable, with a
constant above Taylor--Hood's (\S\ref{sec:beta}), and its face bubbles are
shared between neighbors, so they are global unknowns and do not condense: at
$N = 12$ the enriched space carries $96{,}117$ global displacement degrees of
freedom against $36{,}501$ for $P_2/P_0$ --- and for the composite
tetrahedron --- on the same mesh, a factor of $2.6$, and $2.5$ times
Taylor--Hood's $38{,}698$. The pressure elimination is element by element,
since that depends only on $Q_h$ being discontinuous; the displacement space
is what grows.

Whether the accuracy of a stable pair on coarser meshes compensates for that
factor of $2.6$ is not established, and it is the comparison that decides
...
matters here. The three with a volumetric energy that is not quadratic require
one projection more (Remark~\ref{rem:nonquadratic}), not a local Newton
iteration. What remains outside the split altogether is
...
families of methods carry --- and that claim carries only the weight of the
numerical evidence supporting it. Face-hybridized methods
\citep{AbbasErnPignet:2019} remain the alternative to evaluate if it fails.
