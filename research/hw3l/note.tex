...
for large-deformation deviatoric plasticity, aimed at removing the
...
map. Three fields --- the motion, a volumetric strain and a pressure --- are
...
sequence against a control pair of established stability: of six element-local
pairs only the three-dimensional Crouzeix--Raviart pair --- quadratic
displacement enriched with face and interior bubbles, over a linear
discontinuous pressure --- is stable, at a constant of $0.2968$; its face
bubbles are global, and the pair has $2.6$ times the degrees of freedom of the
composite tetrahedron. The second holds in elasticity for every conforming
pair, and under a plastic tangent that removes the fraction $\eta$ of the
...
pair has no spurious mode among its twenty softest at the finest mesh, and the
constant-pressure pairs retain between five and fifteen. On deformed
configurations the inf-sup constant decreases with the shear of the
deformation and is unchanged by its volume change. A fourteen-point
...
The smoothness of the equivalent plastic strain in a nonlinear solve is not
measured here.
\end{abstract}
...
displacement the pressure does not resist be free, whose failure is a soft
mode.
...
\eqref{eq:energy}, and the two halves of it are requirements of different
...
Table~\ref{tab:split} is that test, run over every material implemented in
Norma and Carina, the two finite-element codes in which the formulation is to
be implemented.
...
Remark~\ref{rem:thetachoice} the choice changes the element: the choice of
$\theta$ changes the discrete element and not only the computation.
\end{remark}
...
quantity of the formulation that depends on the instance; Table~\ref{tab:instances}
...
\paragraph{The pairing at $\F = \Id$ is instance-independent, and
\S\ref{sec:results} relies on this.} By \eqref{eq:dtheta} and the normalization
...
unchanged: those results are a property of the pair $(U_h, Q_h)$ and do not
depend on the instance on which the measurement is performed.

\paragraph{The instances are not interchangeable in plasticity, and this is the
...
algebraically linear in the strain that enters the return map, which is what lets
...
With $\disp \in \mathcal{U}$, $\bar\theta, \bar p \in \mathcal{Q}$, and test
functions $\testd \in \mathcal{U}$ vanishing on $\partial_\disp\Omega$, the
part of the boundary on which the motion is prescribed, and
...
proportional loading. Equation~\eqref{eq:P-general} is the form to
...
\paragraph{constant} discontinuous pressure is not controlled by $P_2$: ``to control
...
is what the linear pressure is for.
...
This section develops the logarithmic instance of
...
continuum and none in the element; once the tangent removes its shear energy
it has zero energy in the element and the energy $\kappa \int (\Div ~v)^2$ in
the continuum. That is the soft
...
its positivity is not guaranteed, and HRZ lumping is used.
...
length, is $0.48$ for \code{TETRA4}, $0.20$ for \code{TETRA10} under HRZ,
and for the enriched element $0.15$ with the nodal basis under HRZ and $0.11$
with the hierarchical basis under Lagrange-only HRZ; on a Kuhn tetrahedron of
the Freudenthal mesh (\S\ref{sec:method}), $0.28$, $0.13$, $0.095$ and
$0.078$. The quadratic element therefore reduces the stable step by a factor
of $2.4$ against the linear one on the same mesh, and the enrichment a further
$1.4$ with the nodal basis. A $P_2$ mesh resolves a given feature with fewer
...
This section gives the extension to an independent deviatoric strain, should
one be needed. Should an independent deviatoric strain field become necessary --- for
...
leaves the six rigid-body modes. That is an element-level fact and no
...
\begin{remark}
This settles the mechanism and nothing more. Inf-sup stability is a
...
only $z = 0$ fixed. The second is closer to the conditions of a body confined
...
The exact-zero soft modes of a constant pressure therefore do not persist
after assembly on these elements. What this test does not detect is the mode
...
which must be one; ``local rate'' is $\ln(\eta_N/\eta_{N+1})/\ln((N+1)/N)$
...
their own, and the row is reported as consistent with stable; its
...
$\beta_h$, and by that little, because the bubble's columns of $~G$ against
$P_0$ are zero.
...
\eqref{eq:cep} at $\eta = 1$, all-Dirichlet Freudenthal cube: the eigenvalue
...
rules supplied by \code{ReferenceFiniteElements}, the Julia package that
...
weights, $4.6$ times fewer points than the exact rule --- and is used, with a
stiffer interior bubble.
...
basis is used: a node-based degree-of-freedom manager carries it without a
new kind of unknown, its mass lumps correctly by the standard construction,
...
null space at every mesh, and every other element-local pair decays. The face
bubbles are the necessary part of the enrichment; the interior bubble alone is
...
$180^\circ$, is unchanged by a volume change of $2.3$, and at each fixed
...
positive masses and conserves momentum, and the Courant number is $0.145$
against $0.476$ for \code{TETRA4} on the same element, a factor of $3.3$, of
which $2.4$ is the quadratic element and $1.4$ the enrichment
(\S\ref{sec:basis}). The comparison at equal accuracy, on the coarser mesh a
...
measurement of \S\ref{sec:verify}. Face-hybridized methods
\citep{AbbasErnPignet:2019} are the alternative should that measurement not
confirm it.
